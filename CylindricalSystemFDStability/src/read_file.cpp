...
void read_file::readInputFile(char *filename, some_data &dat) {
	...
	if (fid == NULL) {
		...
		valuesWritten = sscanf(nextLine, "%lg", &(dat.h));
		if (valuesWritten != 1) {
			fileReadErrorFlag = true;
			goto fileClose;
		}
		...
		// read in NstepsPre
		getNextDataLine(fid, nextLine, MAXLINE, &endOfFileFlag);
		valuesWritten = sscanf(nextLine, "%u", &(dat.NstepsPre));
		if (valuesWritten != 1) {
			fileReadErrorFlag = true;
			goto fileClose;
		}
		// read in Nstep
		getNextDataLine(fid, nextLine, MAXLINE, &endOfFileFlag);
		valuesWritten = sscanf(nextLine, "%u", &(dat.Nsteps));
		if (valuesWritten != 1) {
			fileReadErrorFlag = true;
			goto fileClose;
		}
		...
		// read in NFmode
		getNextDataLine(fid, nextLine, MAXLINE, &endOfFileFlag);
		valuesWritten = sscanf(nextLine, "%u", &(dat.NFmode));
		if (valuesWritten != 1) {
			fileReadErrorFlag = true;
			goto fileClose;
		}
