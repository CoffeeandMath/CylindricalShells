...
void ElasticProblem::setup_system()
{
	...
	stability_solution.reinit(dof_handler.n_dofs());
	linearsolve.reinit(dof_handler.n_dofs());
	system_rhs.reinit(dof_handler.n_dofs());
	stability_rhs.reinit(dof_handler.n_dofs());
	...
	stability_matrix.reinit(sparsity_pattern);
...
void ElasticProblem::assemble_stability_system(){
	...
	std::vector<double> ur_q(n_q_points);
	std::vector<double> uz_q(n_q_points);
	std::vector<double> lambdar_q(n_q_points);
	std::vector<double> lambdaz_q(n_q_points);
	std::vector<double> uxi_r_q(n_q_points);
	std::vector<double> uxi_z_q(n_q_points);
	std::vector<Tensor<1,DIM>> dur_q(n_q_points);
	std::vector<Tensor<1,DIM>> duz_q(n_q_points);
	std::vector<Tensor<1,DIM>> ddur_q(n_q_points);
	std::vector<Tensor<1,DIM>> dduz_q(n_q_points);
	...
	for (const auto &cell : dof_handler.active_cell_iterators())
	{
		...
		fe_values[r].get_function_values(stability_solution,ur_q);
		fe_values[z].get_function_values(stability_solution,uz_q);
		fe_values[r].get_function_gradients(stability_solution,dur_q);
		fe_values[z].get_function_gradients(stability_solution,duz_q);
		fe_values[xi_r].get_function_values(stability_solution,uxi_r_q);
		fe_values[xi_z].get_function_values(stability_solution,uxi_z_q);
		fe_values[xi_r].get_function_gradients(stability_solution,ddur_q);
		fe_values[xi_z].get_function_gradients(stability_solution,dduz_q);
		fe_values[lambda_r].get_function_values(stability_solution,lambdar_q);
		fe_values[lambda_z].get_function_values(stability_solution,lambdaz_q);
		...
		for (const unsigned int q_index : fe_values.quadrature_point_indices()){
			...
			for (const unsigned int i : fe_values.dof_indices())
			{
				...
				for (const unsigned int j : fe_values.dof_indices()) {
					...
					cell_matrix(i,j) += 0.*4.*mu*(ur_q[q_index]*R_i_q + uz_q[q_index]*Z_i_q)*(ur_q[q_index]*R_j_q + uz_q[q_index]*Z_j_q)*fe_values.JxW(q_index);
					cell_matrix(i,j) += 0.*2.*mu*(pow(ur_q[q_index],2) + pow(uz_q[q_index],2) - 1.)*(R_i_q*R_j_q + Z_i_q*Z_j_q)*fe_values.JxW(q_index);
					//*/
				}

				double ur = ur_q[q_index];
				double dur = dur_q[q_index][0];
				double ddur = ddur_q[q_index][0];
				double uz = uz_q[q_index];
				double duz = duz_q[q_index][0];
				double dduz = dduz_q[q_index][0];

				Tensor<2,2> d_CovariantMetric_u_q;
				d_CovariantMetric_u_q[0][0] = rp*dur + zp*duz;
				d_CovariantMetric_u_q[1][1] = rq*ur/pow(Rq,2);

				Tensor<2,2> d_Covariant2Form_u_q;
				d_Covariant2Form_u_q[0][0] = (5.*rp*rpp*zp - 4.*pow(rp,2)*zpp + pow(zp,2)*zpp)/pow(pow(rp,2) + pow(zp,2),3.5) * dur
						-(zp/pow(pow(rp,2) + pow(zp,2),2.5)) * ddur
						+(-(pow(rp,2)*rpp) + 4.*rpp*pow(zp,2) - 5.*rp*zp*zpp)/pow(pow(rp,2) + pow(zp,2),3.5) * duz
						+rp/pow(pow(rp,2) + pow(zp,2),2.5) * dduz;

				d_Covariant2Form_u_q[1][1] = (-3.*pow(Rq,2)*zp)/(pow(rq,4)*sqrt(pow(rp,2) + pow(zp,2)))*ur
						-((rp*pow(Rq,2)*zp)/(pow(rq,3)*pow(pow(rp,2) + pow(zp,2),1.5))) * dur
						+(pow(rp,2)*pow(Rq,2))/(pow(rq,3)*pow(pow(rp,2) + pow(zp,2),1.5)) * duz;

				Tensor<2,2> dd_CovariantMetric_ui_q;
				dd_CovariantMetric_ui_q[0][0] = Di_rp*dur + Di_zp*duz;
				dd_CovariantMetric_ui_q[1][1] = Di_r*ur/pow(Rq,2);
				...
				Tensor<2,2> dd_Covariant2Form_ui_q;
				dd_Covariant2Form_ui_q[0][0] = du*ddb11mat*di;
				dd_Covariant2Form_ui_q[1][1] = d2u*ddb22mat*d2i;
				/*
					cell_rhs(i) += ((dr_q[q_index][0]*dR_i_q[0] + dz_q[q_index][0]*dZ_i_q[0]) * // phi_i(x_q)
							fe_values.JxW(q_index));            // dx
				...
				//Contribution of the in plane stretching
				cell_rhs(i) += Rq*( BilinearProduct(d_CovariantMetric_i_q,Material_Vector_InPlane[cell_index].getddQ2ddF(),d_CovariantMetric_u_q) )*fe_values.JxW(q_index);
				cell_rhs(i) += Rq*(Tensor_Inner(Material_Vector_InPlane[cell_index].getdQ2dF(),dd_CovariantMetric_ui_q))*fe_values.JxW(q_index);

				//Contribution of the bending component
				cell_rhs(i) += hsc*Rq*( BilinearProduct(d_Covariant2Form_i_q,Material_Vector_Bending[cell_index].getddQ2ddF(),d_Covariant2Form_u_q) )*fe_values.JxW(q_index);
				cell_rhs(i) += hsc*Rq*(Tensor_Inner(Material_Vector_Bending[cell_index].getdQ2dF(),dd_Covariant2Form_ui_q))*fe_values.JxW(q_index);
				...
				cell_rhs(i) -= lambdar_q[q_index]*(dR_i_q[0] - Xi_r_i_q)*fe_values.JxW(q_index);
				cell_rhs(i) -= Lambda_r_i_q*(dur - uxi_r_q[q_index])*fe_values.JxW(q_index);
				cell_rhs(i) -= lambdaz_q[q_index]*(dZ_i_q[0] - Xi_z_i_q)*fe_values.JxW(q_index);
				cell_rhs(i) -= Lambda_z_i_q*(duz - uxi_z_q[q_index])*fe_values.JxW(q_index);

				cell_rhs(i) += mu*(dR_i_q[0] - Xi_r_i_q)*(dur - uxi_r_q[q_index])*fe_values.JxW(q_index);
				cell_rhs(i) += mu*(dZ_i_q[0] - Xi_z_i_q)*(duz - uxi_z_q[q_index])*fe_values.JxW(q_index);


				cell_rhs(i) += 0.*2.*mu*(pow(ur_q[q_index],2) + pow(uz_q[q_index],2) - 1.)*(ur_q[q_index]*R_i_q + uz_q[q_index]*Z_i_q)*fe_values.JxW(q_index);
				//cell_rhs(i) += hscinv*homog*Z_i_q*(z_q[q_index] - x_q[0])*fe_values.JxW(q_index);
				// */
		...
		for (unsigned int i = 0; i < local_dof_indices.size(); i++) {
			...
			stability_rhs[local_dof_indices[i]] += cell_rhs[i];
			for (unsigned int j = 0; j < local_dof_indices.size(); j++) {
				stability_matrix.add(local_dof_indices[i],local_dof_indices[j],cell_matrix[i][j]);
			}
		}
	}

	constraints.condense(stability_matrix);
	constraints.condense(stability_rhs);
}
...
void ElasticProblem::stability_solve()
{
	/*
	if (stability_solution.norm_sqr() < 1.e-7){
		double solnorm = sqrt(solution.norm_sqr());
		for (unsigned int i = 0; i < solution.size(); i++){
			stability_solution[i] = 0.001*solution[i]/solnorm;
		}
	}
	*/
	...
	linearsolve = 0;
	SparseDirectUMFPACK a_direct;
	a_direct.initialize(stability_matrix);
	a_direct.vmult(linearsolve,stability_rhs);

	constraints.distribute(linearsolve);
	stability_solution.add(-1.0,linearsolve);
...
void ElasticProblem::stability_newton_raphson() {
	std::cout << "solving for new stability mechanism" << std::endl;
	double stepsize = 2.0*tol;
	int cntr = 0;
	while (stepsize > tol) {
		//prev_solution = solution;
		cntr++;
		std::cout << "assembling stability system" << std::endl;
		assemble_stability_system();
		std::cout << "assembled stability system" << std::endl;
		stability_solve();
	...
	std::cout << "Stability Number of Steps: " << cntr << std::endl;
	std::cout << "unorm = " << stability_solution.norm_sqr() << std::endl;
}
